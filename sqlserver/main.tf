...
resource "azurerm_virtual_machine_extension" "sql_disk_setup" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell -ExecutionPolicy Unrestricted -Command \"$ErrorActionPreference='Stop'; $log='C:\\Windows\\Temp\\configure-sql-disks.log'; $err='C:\\Windows\\Temp\\configure-sql-disks.err.txt'; Start-Transcript -Path 'C:\\Windows\\Temp\\configure-sql-disks.transcript.txt' -Force | Out-Null; try { function Get-DiskNumberFromLun([int]$lun){ $dd=Get-CimInstance Win32_DiskDrive | Where-Object { $_.SCSILogicalUnit -eq $lun } | Select-Object -First 1; if($null -eq $dd){ return $null }; return [int]$dd.Index }; function Ensure-LunDrive([int]$lun,[string]$letter,[string]$label){ $diskNumber=$null; for($r=0; $r -lt 30 -and $null -eq $diskNumber; $r++){ $diskNumber=Get-DiskNumberFromLun $lun; if($null -eq $diskNumber){ Start-Sleep -Seconds 10 } }; if($null -eq $diskNumber){ throw \"Disk for LUN $lun not found after retries\" }; $disk=Get-Disk -Number $diskNumber; $existingVol=Get-Volume -DriveLetter $letter -ErrorAction SilentlyContinue; if($existingVol -and $existingVol.DriveType -ne 'CD-ROM'){ $parts=Get-Partition -DriveLetter $letter -ErrorAction SilentlyContinue; if(-not $parts -or $parts.DiskNumber -ne $diskNumber){ throw \"Drive letter $letter already in use by another disk\" } }; if($disk.PartitionStyle -eq 'RAW'){ Initialize-Disk -Number $diskNumber -PartitionStyle GPT -PassThru | Out-Null; $p=New-Partition -DiskNumber $diskNumber -UseMaximumSize -AssignDriveLetter:$false; Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null; Format-Volume -DriveLetter $letter -FileSystem NTFS -NewFileSystemLabel $label -Confirm:`$false -Force | Out-Null } else { $p=(Get-Partition -DiskNumber $diskNumber | Where-Object { $_.Type -ne 'Reserved' } | Sort-Object Size -Descending | Select-Object -First 1); if($null -eq $p){ throw \"No usable partition found on disk $diskNumber (LUN $lun)\" }; if($p.DriveLetter -ne $letter){ Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null } } }; Ensure-LunDrive 0 'F' 'DATA'; Ensure-LunDrive 1 'G' 'LOG'; Ensure-LunDrive 2 'T' 'TEMPDB'; foreach($p in @('F:\\Data','G:\\Log','T:\\TempDB')){ if(Test-Path ($p.Substring(0,3))){ New-Item -ItemType Directory -Path $p -Force | Out-Null } }; \"OK $(Get-Date -Format o)\" | Out-File -FilePath $log -Append; Stop-Transcript | Out-Null; exit 0 } catch { $_ | Out-String | Out-File -FilePath $err -Append; try { Stop-Transcript | Out-Null } catch {}; exit 1 }\""
  })

...
resource "azurerm_virtual_machine_extension" "sql_disk_setup" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"$ErrorActionPreference='Stop'; $log='C:\\Windows\\Temp\\configure-sql-disks.log'; $err='C:\\Windows\\Temp\\configure-sql-disks.err.txt'; try { $null = Start-Transcript -Path 'C:\\Windows\\Temp\\configure-sql-disks.transcript.txt' -Force; function Ensure-Drive([int]$diskNumber,[string]$letter,[string]$label){ $disk = Get-Disk -Number $diskNumber; $existingVol = Get-Volume -DriveLetter $letter -ErrorAction SilentlyContinue; if($existingVol){ $partsByLetter = Get-Partition -DriveLetter $letter -ErrorAction SilentlyContinue; if(-not $partsByLetter -or $partsByLetter[0].DiskNumber -ne $diskNumber){ throw \"Drive letter $letter already in use by another disk\" } }; if($disk.PartitionStyle -eq 'RAW'){ $null = Initialize-Disk -Number $diskNumber -PartitionStyle GPT; $p = New-Partition -DiskNumber $diskNumber -UseMaximumSize -AssignDriveLetter:$false; $null = Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter; $null = Format-Volume -DriveLetter $letter -FileSystem NTFS -NewFileSystemLabel $label -Confirm:`$false -Force } else { $parts = Get-Partition -DiskNumber $diskNumber -ErrorAction SilentlyContinue; $usable = @(); foreach($pp in $parts){ if($pp.Type -ne 'Reserved'){ $usable += $pp } }; $usable = Sort-Object -InputObject $usable -Property Size -Descending; if($usable.Count -lt 1){ throw \"No usable partition found on disk $diskNumber\" }; $p = $usable[0]; if($p.DriveLetter -ne $letter){ $null = Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter } } }; $osNums=@(); foreach($d in Get-Disk){ if($d.IsBoot -or $d.IsSystem){ $osNums += [int]$d.Number } }; $candidates=@(); foreach($dd in Get-CimInstance Win32_DiskDrive){ $idx=[int]$dd.Index; if($osNums -contains $idx){ continue }; if($null -eq $dd.SCSILogicalUnit){ continue }; $candidates += [pscustomobject]@{ Number=$idx; Lun=[int]$dd.SCSILogicalUnit; Model=$dd.Model; Size=$dd.Size } }; $candidates = Sort-Object -InputObject $candidates -Property Lun,Number; $lines=@(); foreach($c in $candidates){ $lines += (\"DiskNumber=\"+$c.Number+\" Lun=\"+$c.Lun+\" Model=\"+$c.Model+\" Size=\"+$c.Size) }; Add-Content -Path $log -Value $lines; $lunToLetter=@{0='F';1='G';2='T'}; $lunToLabel=@{0='DATA';1='LOG';2='TEMPDB'}; foreach($lun in 0,1,2){ $row=$null; foreach($c in $candidates){ if($c.Lun -eq $lun){ $row=$c; break } }; if($row){ Ensure-Drive $row.Number $lunToLetter[$lun] $lunToLabel[$lun] } }; if(-not (Test-Path 'F:\\') -or -not (Test-Path 'G:\\') -or -not (Test-Path 'T:\\')){ $fallback = Sort-Object -InputObject (Get-Disk | Where-Object { -not $_.IsBoot -and -not $_.IsSystem }) -Property Number; if($fallback.Count -ge 3){ Ensure-Drive $fallback[0].Number 'F' 'DATA'; Ensure-Drive $fallback[1].Number 'G' 'LOG'; Ensure-Drive $fallback[2].Number 'T' 'TEMPDB' } else { throw \"Not enough data disks detected (need 3).\" } }; if(Test-Path 'F:\\'){ $null = New-Item -ItemType Directory -Path 'F:\\Data' -Force }; if(Test-Path 'G:\\'){ $null = New-Item -ItemType Directory -Path 'G:\\Log' -Force }; if(Test-Path 'T:\\'){ $null = New-Item -ItemType Directory -Path 'T:\\TempDB' -Force }; Add-Content -Path $log -Value (\"OK \"+(Get-Date -Format o)); try { $null = Stop-Transcript } catch {}; exit 0 } catch { $msg = $_.Exception.Message; $stack = $_.ScriptStackTrace; Add-Content -Path $err -Value (\"ERROR \"+(Get-Date -Format o)+\" :: \"+$msg); if($stack){ Add-Content -Path $err -Value $stack }; try { $null = Stop-Transcript } catch {}; exit 1 }\""
  })

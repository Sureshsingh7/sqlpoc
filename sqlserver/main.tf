...
resource "azurerm_virtual_machine_extension" "sql_disk_setup" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell -ExecutionPolicy Unrestricted -Command \"$ErrorActionPreference='Stop'; try { $letters=@('F','G','T'); $dataDisks=(Get-Disk | Where-Object { -not $_.IsBoot -and -not $_.IsSystem } | Sort-Object Number); for($i=0; $i -lt [Math]::Min($dataDisks.Count,$letters.Count); $i++){ $disk=$dataDisks[$i]; $letter=$letters[$i]; if($disk.PartitionStyle -eq 'RAW'){ Initialize-Disk -Number $disk.Number -PartitionStyle GPT -PassThru | New-Partition -UseMaximumSize -DriveLetter $letter | Format-Volume -FileSystem NTFS -NewFileSystemLabel $letter -Confirm:`$false -Force | Out-Null } else { $part=(Get-Partition -DiskNumber $disk.Number | Where-Object { $_.DriveLetter -eq $null -and $_.Type -ne 'Reserved' } | Select-Object -First 1); if($part){ Set-Partition -DiskNumber $disk.Number -PartitionNumber $part.PartitionNumber -NewDriveLetter $letter | Out-Null } } }; if(Test-Path 'F:\\'){ New-Item -ItemType Directory -Path 'F:\\Data' -Force | Out-Null }; if(Test-Path 'G:\\'){ New-Item -ItemType Directory -Path 'G:\\Log' -Force | Out-Null }; if(Test-Path 'T:\\'){ New-Item -ItemType Directory -Path 'T:\\TempDB' -Force | Out-Null }; 'OK' | Out-File -FilePath 'C:\\Windows\\Temp\\configure-sql-disks.log' -Append; exit 0 } catch { $_ | Out-String | Out-File -FilePath 'C:\\Windows\\Temp\\configure-sql-disks.err.txt' -Append; exit 1 }\""
  })

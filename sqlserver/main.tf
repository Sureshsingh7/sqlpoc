...
resource "azurerm_virtual_machine_extension" "sql_disk_setup" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell -ExecutionPolicy Bypass -Command \"$ErrorActionPreference='Stop'; $log='C:\\Windows\\Temp\\configure-sql-disks.log'; $err='C:\\Windows\\Temp\\configure-sql-disks.err.txt'; Start-Transcript -Path 'C:\\Windows\\Temp\\configure-sql-disks.transcript.txt' -Force | Out-Null; try { function Get-DataDisksWithLun(){ $osDisks = Get-Disk | Where-Object { $_.IsBoot -or $_.IsSystem } | Select-Object -ExpandProperty Number; $cimDisks = Get-CimInstance -Namespace root/Microsoft/Windows/Storage -ClassName MSFT_Disk; $rows = foreach($cd in $cimDisks){ $num = [int]$cd.Number; if($osDisks -contains $num){ continue }; $m = [regex]::Match($cd.Location, 'LUN\\s+(\\d+)'); if(-not $m.Success){ continue }; [pscustomobject]@{ Number=$num; Lun=[int]$m.Groups[1].Value; Location=$cd.Location } }; $rows | Sort-Object Lun, Number }; function Ensure-Drive([int]$diskNumber,[string]$letter,[string]$label){ $disk=Get-Disk -Number $diskNumber; $existingVol=Get-Volume -DriveLetter $letter -ErrorAction SilentlyContinue; if($existingVol -and $existingVol.DriveType -ne 'CD-ROM'){ $parts=Get-Partition -DriveLetter $letter -ErrorAction SilentlyContinue; if(-not $parts -or $parts.DiskNumber -ne $diskNumber){ throw \"Drive letter $letter already in use by another disk\" } }; if($disk.PartitionStyle -eq 'RAW'){ Initialize-Disk -Number $diskNumber -PartitionStyle GPT -PassThru | Out-Null; $p=New-Partition -DiskNumber $diskNumber -UseMaximumSize -AssignDriveLetter:$false; Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null; Format-Volume -DriveLetter $letter -FileSystem NTFS -NewFileSystemLabel $label -Confirm:`$false -Force | Out-Null } else { $p=(Get-Partition -DiskNumber $diskNumber | Where-Object { $_.Type -ne 'Reserved' } | Sort-Object Size -Descending | Select-Object -First 1); if($null -eq $p){ throw \"No usable partition found on disk $diskNumber\" }; if($p.DriveLetter -ne $letter){ Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null } } }; $data=$null; for($r=0; $r -lt 30; $r++){ $data = Get-DataDisksWithLun; if(($data | Where-Object { $_.Lun -in 0,1,2 }).Count -ge 3){ break }; Start-Sleep -Seconds 10 }; \"Detected data disks: $($data | ConvertTo-Json -Compress)\" | Out-File -FilePath $log -Append; $map=@{0='F';1='G';2='T'}; foreach($lun in 0,1,2){ $row = $data | Where-Object { $_.Lun -eq $lun } | Select-Object -First 1; if($null -eq $row){ throw \"Data disk with LUN $lun not found (Location parse failed).\" }; Ensure-Drive $row.Number $map[$lun] ( $lun -eq 0 ? 'DATA' : ($lun -eq 1 ? 'LOG' : 'TEMPDB') ) }; foreach($p in @('F:\\Data','G:\\Log','T:\\TempDB')){ if(Test-Path ($p.Substring(0,3))){ New-Item -ItemType Directory -Path $p -Force | Out-Null } }; \"OK $(Get-Date -Format o)\" | Out-File -FilePath $log -Append; Stop-Transcript | Out-Null; exit 0 } catch { $_ | Out-String | Out-File -FilePath $err -Append; try { Stop-Transcript | Out-Null } catch {}; exit 1 }\""
  })

...
resource "azurerm_virtual_machine_extension" "sql_disk_setup" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"$ErrorActionPreference='Stop'; $log='C:\\Windows\\Temp\\configure-sql-disks.log'; $err='C:\\Windows\\Temp\\configure-sql-disks.err.txt'; Start-Transcript -Path 'C:\\Windows\\Temp\\configure-sql-disks.transcript.txt' -Force | Out-Null; try { function Get-DataDisksWithLun(){ $osDiskNumbers=@(); foreach($d in (Get-Disk)){ if($d.IsBoot -or $d.IsSystem){ $osDiskNumbers += [int]$d.Number } }; $cimDisks = Get-CimInstance -Namespace root/Microsoft/Windows/Storage -ClassName MSFT_Disk; $rows=@(); foreach($cd in $cimDisks){ $num=[int]$cd.Number; if($osDiskNumbers -contains $num){ continue }; $m=[regex]::Match($cd.Location,'LUN\\s+(\\d+)'); if(-not $m.Success){ continue }; $rows += [pscustomobject]@{ Number=$num; Lun=[int]$m.Groups[1].Value; Location=$cd.Location } }; $rows = Sort-Object -InputObject $rows -Property Lun,Number; return $rows }; function Ensure-Drive([int]$diskNumber,[string]$letter,[string]$label){ $disk=Get-Disk -Number $diskNumber; $existingVol=Get-Volume -DriveLetter $letter -ErrorAction SilentlyContinue; if($existingVol -and $existingVol.DriveType -ne 'CD-ROM'){ $parts=Get-Partition -DriveLetter $letter -ErrorAction SilentlyContinue; if(-not $parts -or $parts[0].DiskNumber -ne $diskNumber){ throw \"Drive letter $letter already in use by another disk\" } }; if($disk.PartitionStyle -eq 'RAW'){ Initialize-Disk -Number $diskNumber -PartitionStyle GPT -PassThru | Out-Null; $p=New-Partition -DiskNumber $diskNumber -UseMaximumSize -AssignDriveLetter:$false; Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null; Format-Volume -DriveLetter $letter -FileSystem NTFS -NewFileSystemLabel $label -Confirm:`$false -Force | Out-Null } else { $parts=Get-Partition -DiskNumber $diskNumber -ErrorAction SilentlyContinue; $usable=@(); foreach($pp in $parts){ if($pp.Type -ne 'Reserved'){ $usable += $pp } }; $usable = Sort-Object -InputObject $usable -Property Size -Descending; if($usable.Count -lt 1){ throw \"No usable partition found on disk $diskNumber\" }; $p=$usable[0]; if($p.DriveLetter -ne $letter){ Set-Partition -DiskNumber $diskNumber -PartitionNumber $p.PartitionNumber -NewDriveLetter $letter | Out-Null } } }; $data=$null; for($r=0; $r -lt 30; $r++){ $data = Get-DataDisksWithLun; $countWanted=0; foreach($row in $data){ if($row.Lun -eq 0 -or $row.Lun -eq 1 -or $row.Lun -eq 2){ $countWanted++ } }; if($countWanted -ge 3){ break }; Start-Sleep -Seconds 10 }; foreach($lun in 0,1,2){ $row=$null; foreach($candidate in $data){ if($candidate.Lun -eq $lun){ $row=$candidate; break } }; if($null -eq $row){ throw \"Data disk with LUN $lun not found (Location parse failed).\" }; $letter=''; $label=''; if($lun -eq 0){ $letter='F'; $label='DATA' } elseif($lun -eq 1){ $letter='G'; $label='LOG' } else { $letter='T'; $label='TEMPDB' }; Ensure-Drive $row.Number $letter $label }; foreach($p in @('F:\\Data','G:\\Log','T:\\TempDB')){ if(Test-Path ($p.Substring(0,3))){ New-Item -ItemType Directory -Path $p -Force | Out-Null } }; \"OK $(Get-Date -Format o)\" | Out-File -FilePath $log -Append; Stop-Transcript | Out-Null; exit 0 } catch { $_ | Out-String | Out-File -FilePath $err -Append; try { Stop-Transcript | Out-Null } catch {}; exit 1 }\""
  })

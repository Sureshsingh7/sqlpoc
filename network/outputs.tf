...
output "nsg_sql1_id" {
  value = azurerm_network_security_group.nsg_sql1.id
}

output "nsg_sql2_id" {
  value = azurerm_network_security_group.nsg_sql2.id
}

output "nsg_runner_id" {
  value = azurerm_network_security_group.nsg_runner.id
}
...
output "dr_sql_vnet_id" {
  value = var.is_dr_enabled ? azurerm_virtual_network.dr_sql[0].id : null
}

output "dr_sql_subnet_sql1_id" {
  value = var.is_dr_enabled ? azurerm_subnet.dr_sql_sql1[0].id : null
}

output "dr_sql_subnet_sql2_id" {
  value = var.is_dr_enabled ? azurerm_subnet.dr_sql_sql2[0].id : null
}

output "dr_pep_subnet_id" {
  value = var.is_dr_enabled ? azurerm_subnet.dr_sql_pep[0].id : null
}
